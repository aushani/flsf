...
void FlowProcessor::Update(const kt::VelodyneScan &scan, bool copy_data) {
  ...
  // Compute flow
  data_->flow_image = data_->solver.ComputeFlow(data_->distance, p_background, data_->resolution, iterations_);
...
void FlowProcessor::Refresh() {
  ...
  // Compute flow
  auto fi = data_->solver.ComputeFlow(data_->distance, data_->p_background1, data_->resolution, iterations_);
